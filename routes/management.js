...
router.get("/management/lang",rateLimit(), (req, res) => {
    ...
    var langJson = {};
    try {
        langJson = JSON.parse(fs.readFileSync("./assets/lang/" + lang.toLocaleLowerCase()+".json", "utf8"));
    } catch (error) {
        return res.status(400).json({status: "Bad Request"});
    }

    if(langJson.Projects == undefined || langJson.Projects == null || langJson.Projects == "") {
        return res.status(400).json({status: "Bad Request"});
    }    
